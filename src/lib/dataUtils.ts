...
export async function loadAirQualityData(): Promise<AirQualityReading[]> {
	try {
		...
		const response = await fetch('/data/african_cities_air_quality_2024_2026.parquet');
		if (!response.ok) {
...
export function calculateCityAverages(data: AirQualityReading[]): Record<string, AirQualityReading> {
	...
	Object.entries(cityGroups).forEach(([city, readings]) => {
		...
		const avgReading: AirQualityReading = {
			...
			overall_aqi: readings.reduce((sum, r) => sum + r.overall_aqi, 0) / readings.length,
...
export function simulateCurrentTime(): Date {
	// Simulate that we're somewhere in the middle of 2024
	// This allows us to show "historical" data as if it's current
	// Set to end of June 15, 2024 to ensure we capture all data for that day
	return new Date('2024-06-15T23:59:59Z');
}
...
export function aggregateDataByTime(
	...
	groupedData.forEach((readings, groupKey) => {
		...
		const avgReading: AirQualityReading = {
			...
			overall_aqi: Number((readings.reduce((sum, r) => sum + Number(r.overall_aqi), 0) / readings.length).toFixed(0)),
